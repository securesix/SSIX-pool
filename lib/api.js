...
async function getPaymentsChartData(address) {
    ...
    var promise = new Promise((resolve, reject) => {
        redisClient.zrevrange(config.coin + ":payments:" + address, 0, -1, "WITHSCORES", function(error, results) {
            if (error) {
                reject(error);
            }
            ...
            for (var i = 0; i < results.length; i += 2) {
                 ...
                 if (results[i+1] < beginAtTimestamp) {
                     break;
                 }
...
function convertPayments(payments) {
    ...
    for (var i in payments.payments) {
       res.unshift([new Date(i).getTime() / 1000, payments.payments[i]]);
    }
    return res;
}
