...
function IsBannedWallet(wallet){
    if (!banningEnabled || !config.poolServer.walletBlacklist || !(wallet in config.poolServer.walletBlacklist)) return false;
...
function Miner(id, login, workerName, pass, ip, remotePort, port, startingDiff, noRetarget, pushMessage){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
...
function handleMinerMethod(method, params, ip, remotePort, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            miner = new Miner(minerId, login, workerName, params.pass, ip, remotePort, port, difficulty, noRetarget, pushMessage);
	    ...
	    if (!noncePattern.test(params.nonce)) {
            ...
            if (miner.noRetarget && (now - miner.lastShareTime) < 5){ // Disable fixed difficulty if time between shares is less than 5 seconds
...
function startPoolServerTcp(callback){
    async.each(config.poolServer.ports, function(portData, cback){
        var handleMessage = function(socket, jsonData, pushMessage){
            ...
            handleMinerMethod(jsonData.method, jsonData.params, socket.remoteAddress, socket.remotePort, portData, sendReply, pushMessage);
        };
