...
Miner.prototype = {
    ...
    getJob: function(){
        if (currentBlockTemplate === undefined || (this.lastBlockHeight === currentBlockTemplate.height && !this.pendingDifficulty)) {
            return {
                blob: '',
                job_id: '',
                target: '',
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
	    ...
	    if (!noncePattern.test(params.nonce)) {
            ...
            if (miner.noRetarget && (now - miner.lastShareTime) < 10){ // Disable fixed difficulty if time between shares is less than 10 seconds
