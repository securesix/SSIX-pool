...
//          log('error', logSystem, 'Error polling getblocktemplate %j', [error]);
        ...
        if (!currentBlockTemplate || result.height > currentBlockTemplate.height || (currentBlockTemplate.num_transactions == 0 && result.num_transactions > 0)){
            log('info', logSystem, 'New block to mine at height %d w/ difficulty of %d (%d transactions)', [result.height, result.difficulty, (result.num_transactions || 0)]);
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                currentBlockTemplate = undefined;
                jobRefresh();
            }
            else{
